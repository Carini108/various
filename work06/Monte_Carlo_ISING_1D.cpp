...
void Move(int metro)
{
  ...
  //Select randomly a particle to flip (for C++ syntax, 0 <= o <= nspin-1)
    ...
    {
      if ( rnd.Rannyu()<min(1,exp(-beta*Boltzmann(s[o], o))) ) 
      {
        s[o]=-s[o];
        accepted+=1.;
      }
    ...
    { 
      double sum_nearest_neighbours = s[Pbc(ip-1)] + s[Pbc(ip+1)];
      accepted+=1.;
      if ( rnd.Rannyu()<pow(1.+exp(-2.*beta*J*s[o]*sum_nearest_neighbours),-1.) ) 
      {;}
      else 
      {
       s[o]=-s[o];
